...
+ (void)showWith:(UIView *)superView title:(NSString *)title {
    ...
    // 设置父视图
    if (!superView) {
        if ([UIApplication sharedApplication].windows.lastObject.hidden) {
            [ProgressHUD defaultManager].hudSuperView = [UIApplication sharedApplication].keyWindow;
        } else {
            [ProgressHUD defaultManager].hudSuperView = [UIApplication sharedApplication].windows.lastObject;
        }
    } else {
